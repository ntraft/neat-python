...
def test_feed_forward_layers():
    ...
    connections = [(0, 2), (1, 2), (1, 3), (2, 3), (2, 4), (3, 4)]
    layers = feed_forward_layers(inputs, outputs, connections)
    assert [{2}, {3}, {4}] == layers, f"actually got: {layers}"

    # A graph with dangling input nodes: node 2 has no dependencies, yet some other nodes depend upon it.
    inputs = [0, 1]
    outputs = [4]
    connections = [(1, 3), (2, 3), (2, 4), (3, 4)]
    layers = feed_forward_layers(inputs, outputs, connections)
    assert [{2}, {3}, {4}] == layers, f"actually got: {layers}"
    ...
    connections = [(0, 4), (1, 4), (1, 5), (2, 5), (2, 6), (3, 6), (3, 7),
                   ...
                   (10, 12), (10, 13)]
    layers = feed_forward_layers(inputs, outputs, connections)
    assert [{4, 5, 6}, {8, 7}, {9, 11}, {10}, {12, 13}] == layers, f"actually got: {layers}"

    # Another graph with dangling input nodes (node 5).
    inputs = [0, 1, 2, 3]
    outputs = [11, 12, 13]
    connections = [(0, 4), (1, 4), (2, 6), (3, 6), (3, 7),
                   (4, 8), (5, 8), (5, 9), (5, 10), (6, 10), (6, 7),
                   (8, 11), (8, 12), (8, 9), (9, 10), (7, 10),
                   (10, 12), (10, 13)]
    layers = feed_forward_layers(inputs, outputs, connections)
    assert [{5}, {4, 6}, {8, 7}, {9, 11}, {10}, {12, 13}] == layers, f"actually got: {layers}"

    # A graph with dangling output nodes.
    inputs = [0, 1, 2, 3]
    outputs = [11, 12, 13]
    connections = [(0, 4), (1, 4), (1, 5), (2, 5), (2, 6), (3, 6), (3, 7),
                   (4, 8), (5, 8), (5, 9), (5, 10), (6, 10), (6, 7),
